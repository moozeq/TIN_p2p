...
void SendFileTcp::execute(void) {
	size_t ownerId = NetMainThread::getNodeInfo()->getOwnerId(msg.hash, noMutex);
	NetMainThread::getNodeInfo()->registerFileTransfer(msg.hash, noMutex);
	std::string fileStr;
	...
	NetMainThread::getNodeInfo()->unregisterFileTransfer(msg.hash, noMutex);
}
